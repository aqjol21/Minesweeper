...
public class LevelC extends Application {
    ...
    private static Image image7 = new Image("sample/draw.png");
    ...
    Parent restart(){
        ...
        System.out.println("NumOfBombs: " + numberOfBombs);
    ...
    private void winAlert(){
        ...
        if(x == null){
            ...
            ImageView flag = new ImageView(image7);
    ...
    private void openBombs(Position grid[][]) {
        for (int i = 0; i < rowSize; i++) {
            for (int j = 0; j < columnSize; j++) {
                if (grid[i][j].hasBomb) {
    ...
    private void countBomb(Position grid[][]) {
        for (int i = 0; i < rowSize; i++) {
            for (int j = 0; j < columnSize; j++) {
                if (grid[i][j].hasBomb) {
                    numberOfBombs++;
    ...
    class Position extends Pane{
        ...
        private boolean checkForWin(Position[][] tile, int row, int col){
            boolean win = true;
            for(int i = 0;i < row;i++){
                for(int j = 0;j < col;j++){
